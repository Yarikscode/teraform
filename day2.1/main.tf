...
locals {
  ...
  subnet_cidr = cidrsubnet(local.base_cidr, 8, random_integer.cidr_offset.result)
}
...
resource "yandex_compute_instance" "this" {
  ...
  resources {
    cores  = tonumber(var.instance_resources_cores)
    memory = tonumber(var.instance_resources_memory)
  }
...
resource "yandex_storage_bucket" "this" {
  bucket     = join("", [local.bucket_name, "nikola"])
